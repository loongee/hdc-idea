...
intellijPlatform {
    pluginConfiguration {
        name = if (buildHdcVersion) "hdc_idea" else "adb_idea"
...
// 根据构建类型选择不同的 plugin.xml
tasks.processResources {
    if (buildHdcVersion) {
        // HDC 版本: 使用 plugin-hdc.xml
        exclude("META-INF/plugin.xml")
        rename("plugin-hdc.xml", "plugin.xml")
    } else {
        // ADB 版本: 排除 HDC 相关的 plugin.xml
        exclude("META-INF/plugin-hdc.xml")
    }
}
...
dependencies {
    intellijPlatform {
        ...
        if (buildHdcVersion) {
            // HDC 版本: 使用 IntelliJ IDEA Community 作为基础平台
            // DevEco Studio 基于 IntelliJ Platform
            if (project.hasProperty("localIdeOverride")) {
                local(property("localIdeOverride").toString())
            } else {
                intellijIdeaCommunity(property("ideVersion").toString())
            }
        } else {
            // ADB 版本: 使用 Android Studio
